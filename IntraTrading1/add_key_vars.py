...
class AddKeyVars:
    def __init__(self, intra_data: pd.DataFrame, daily_data: pd.DataFrame, dividends: pd.DataFrame, rolling_vol: int = 14, rolling_move: int = 14):
        ...
        self.intra_data_['day'] = pd.to_datetime(
            self.intra_data_['caldt']).dt.date
    ...
    def calculate_vwap(curr_day_data):
        hlc = (curr_day_data['high'] +
               curr_day_data['low'] + curr_day_data['close']) / 3
    ...
    def apply_move_metrics(self):
        ...
        for day in range(1, len(self.days)):
            ...
            # VWAP
            self.intra_data_.loc[curr_day_data.index,
                                 'vwap'] = self.calculate_vwap(curr_day_data)

            # MOVE
            open_price = curr_day_data['open'].iloc[0]
            self.intra_data_.loc[curr_day_data.index, 'move_open'] = self.calculate_move_open(
                curr_day_data, open_price)

            # VOLATILITY
            d_ret.loc[curr_day] = curr_day_data['close'].iloc[-1] / \
                prev_day_data['close'].iloc[-1] - 1
            if day >= self.rolling_vol:
                self.intra_data_.loc[curr_day_data.index, 'd_vol'] = self.calculate_volatility(
                    d_ret, curr_day, prev_day, self.rolling_vol)

    def apply_time_metrics(self):
        self.intra_data_['min_from_open'] = (
            (self.intra_data_.index - self.intra_data_.index.normalize()) / pd.Timedelta(minutes=1)) - 9 * 60
        self.intra_data_['min_of_day'] = self.intra_data_[
            'min_from_open'].round().astype(int)
        ...
        self.intra_data_['move_open_rolling_mean'] = minute_groups['move_open'].transform(
            lambda x: x.rolling(window=self.rolling_move,
                                min_periods=self.rolling_move - 1).mean()
        )
        self.intra_data_['sigma_open'] = minute_groups['move_open_rolling_mean'].transform(
            lambda x: x.shift(1))

    def merge_dividends(self):
        self.dividends_['day'] = pd.to_datetime(
            self.dividends_['caldt']).dt.date
        self.intra_data_ = self.intra_data_.merge(
            self.dividends_[['day', 'dividend']], on='day', how='left')
